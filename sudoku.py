...
def Tabuleiro_Hover(window, mouse_position_x, mouse_position_y):
    ...
    pg.draw.rect(window, salmon, (0, 0, 1000, 700))
    if x >= 0 and x <= 8 and y >= 0 and y <= 8:
        pg.draw.rect(window, dark_red, ((ajuste + x * quadrado, ajuste + y * quadrado, quadrado, quadrado)))
...
def Celula_Selecionada(window, mouse_position_x, mouse_position_y, click_last_status, click, x, y):
    ...
    if click_last_status == True and click == True:
        ...
        y = (math.ceil((mouse_position_y - ajuste) / quadrado) - 1)
    if x >= 0 and x <= 8 and y >= 0 and y <= 8:
        pg.draw.rect(window, dark_red1, ((ajuste + x * quadrado, ajuste + y * quadrado, quadrado, quadrado)))
    return x, y
...
def Boatao_Restart(window):
    pg.draw.rect(window, red_orange, (700, 60, 200, 80))
    palavra_f = fonte.render('Gerar', True, preto)
